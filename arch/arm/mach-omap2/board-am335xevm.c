...
struct da8xx_lcdc_platform_data INNOLUX_TN92_pdata = {
       .manu_name              = "INNOLUX",
       .controller_data        = &lcd_cfg,
       .type                   = "INNOLUX_TN92",
};

struct da8xx_lcdc_platform_data LSA40AT9001_pdata = {
       .manu_name              = "CHIMEI",
       .controller_data        = &lcd_cfg,
       .type                   = "LSA40AT9001",
};
...
static struct pinmux_config mcasp0_pin_mux[] = {
	...
	{"mcasp0_ahclkr.mcasp0_axr2", OMAP_MUX_MODE2 | AM33XX_PIN_INPUT_PULLUP},
	{NULL, 0},
};

/* Module pin mux for BeagleBone Black mcasp0 */
...
/* pinmux for Chipsee Beaglebone Expansion Board V-3 gpio based key */
static struct pinmux_config chipsee_gpio_keys_pin_mux[] = {
...
static struct pinmux_config chipsee_gpio_led_mux[] = {
       {"gpmc_ben0_cle.gpio2_5", OMAP_MUX_MODE7 | AM33XX_PIN_INPUT},
       {"gpmc_wen.gpio2_4", OMAP_MUX_MODE7 | AM33XX_PIN_INPUT},
       {NULL, 0},
};

static struct pinmux_config chipsee_buzz_mux[] = {
       {"gpmc_oen_ren.gpio2_3", OMAP_MUX_MODE7 | AM33XX_PIN_INPUT},
       {NULL, 0},
};

/* pinmux for ADXL345 on Chipsee Beaglebone Black Expansion Board */
static struct pinmux_config chipsee_sensors_pin_mux[] = {
       {"gpmc_clk.gpio2_1",  OMAP_MUX_MODE7 | AM33XX_PIN_INPUT},
       {NULL, 0},
};

/* pinmux for FT5X06 touchscreen on Chipsee Beaglebone Black Expansion Board */
static struct pinmux_config cap_tsc_pin_mux[] = {
        {"gpmc_a1.gpio1_17", OMAP_MUX_MODE7 | AM33XX_PIN_INPUT},
        {NULL, 0},
};

/* pinmux for Audio Enable */
static struct pinmux_config chipsee_audio_enable_pin_mux[] = {
        {"mcasp0_aclkr.gpio3_18", OMAP_MUX_MODE7 | AM33XX_PIN_INPUT_PULLUP},
        {NULL, 0},
};
...
static struct adxl34x_platform_data adxl34x_info = {
        .data_rate = 0x08,
        .data_range = ADXL_FULL_RES | ADXL_RANGE_PM_16g,
        .ev_type = EV_ABS,
        .ev_code_x = ABS_X,
        .ev_code_y = ABS_Y,
        .ev_code_z = ABS_Z,
...
#define ADXL345_INT1 GPIO_TO_PIN(2,1)
#define CAP_TSC_INT GPIO_TO_PIN(1,17)
...
/* Configure GPIOs for Chipsee Beaglebone Black Expansion GPIO Keys */
static struct gpio_keys_button chipsee_gpio_buttons[] = {
        {
                .code                   = KEY_HOME,
...
static struct gpio_keys_platform_data chipsee_gpio_key_info = {
        .buttons        = chipsee_gpio_buttons,
        .nbuttons       = ARRAY_SIZE(chipsee_gpio_buttons),
};

static struct platform_device chipsee_gpio_keys = {
        .name   = "gpio-keys",
        .id     = -1,
        .dev    = {
               .platform_data  = &chipsee_gpio_key_info,
        },
};
...
static struct gpio_led chipsee_gpio_leds[] = {
        {
                .name                   = "am335x:Chipsee:mmc0",
                .gpio                   = GPIO_TO_PIN(2, 5),   // USER_LED0
                .default_trigger        = "mmc0",
        },
        {
                .name                   = "am335x:Chipsee:heartbeat",
                .gpio                   = GPIO_TO_PIN(2, 4),   // USER_LED1
                .default_trigger        = "heartbeat",
        },
};

static struct gpio_led_platform_data chipsee_gpio_led_info = {
        .leds           = chipsee_gpio_leds,
        .num_leds       = ARRAY_SIZE(chipsee_gpio_leds),
};

static struct platform_device chipsee_leds_gpio = {
        .name   = "leds-gpio",
        .id     = -1,
        .dev    = {
...
#define AUDIO_ENABLE GPIO_TO_PIN(3, 18)
#define BUZZ_GPIO GPIO_TO_PIN(2,3)

static void chipsee_init(int evm_id, int profile)
{
	// Capacitive touchscreen
	setup_pin_mux(cap_tsc_pin_mux);	

	// Sensors
	setup_pin_mux(chipsee_sensors_pin_mux);

	// Audio Amplifier
	setup_pin_mux(chipsee_audio_enable_pin_mux);
        gpio_request(AUDIO_ENABLE , "");
        gpio_direction_output(AUDIO_ENABLE, 1);

	// Buzzer
        setup_pin_mux(chipsee_buzz_mux);
        gpio_request(BUZZ_GPIO, "sysfs");
        gpio_export(BUZZ_GPIO, 1);
        gpio_direction_output(BUZZ_GPIO, 0);

	// GPIO keys
        setup_pin_mux(chipsee_gpio_keys_pin_mux);
        platform_device_register(&chipsee_gpio_keys);

	// GPIO leds
	setup_pin_mux(chipsee_gpio_led_mux);
        platform_device_register(&chipsee_leds_gpio);
...
static struct evm_dev_cfg beaglebone_black_dev_cfg[] = {
	...
	{enable_ehrpwm1,	DEV_ON_BASEBOARD, PROFILE_NONE},
	...
	{sgx_init,	DEV_ON_BASEBOARD, PROFILE_NONE},
	{chipsee_init,	DEV_ON_BASEBOARD, PROFILE_NONE},
	{NULL, 0, 0},
};
/* EVM - Starter Kit */
